...
class WaveformCNN1D(nn.Module):
    """
    (c) Wen 2018 风格 1D-CNN，输入原始时域波形 (B, 8192)。
    Conv1d(1→16, k=64, s=8) → BN → ReLU → MaxPool(2)
    Conv1d(16→32, k=32, s=4) → BN → ReLU → MaxPool(2)
    Conv1d(32→64, k=16, s=2) → BN → ReLU → MaxPool(2)
    Conv1d(64→128, k=8, s=1) → BN → ReLU → AdaptiveAvgPool(1)
    FC(128→64) → ReLU → Dropout(0.3) → FC(64→2)
    """
    def __init__(self, in_len=8192, num_classes=NUM_CLASSES):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv1d(1, 16,  kernel_size=64, stride=8, padding=28),
            nn.BatchNorm1d(16), nn.ReLU(True), nn.MaxPool1d(2),
...
def main():
    ...
    # ════════════ (c) 1D-CNN on raw waveform ════════════
    print("\n[3/4] 加载原始时域波形 (8,192 维) ...")
    Xw_tr, yw_tr = load_raw_waveforms(TRAIN_DIRS)
    Xw_va, yw_va = load_raw_waveforms(VAL_DIRS)
    Xw_te, yw_te = load_raw_waveforms(TEST_DIRS)
    print(f"  Train={Xw_tr.shape[0]}  Val={Xw_va.shape[0]}  Test={Xw_te.shape[0]}")

    dl_tr_c = DataLoader(VecDataset(Xw_tr,yw_tr), batch_size=32, shuffle=True,  num_workers=0)
    dl_va_c = DataLoader(VecDataset(Xw_va,yw_va), batch_size=64, shuffle=False, num_workers=0) if Xw_va.shape[0]>0 else None
    dl_te_c = DataLoader(VecDataset(Xw_te,yw_te), batch_size=64, shuffle=False, num_workers=0) if Xw_te.shape[0]>0 else None

    print("\n[4/4] 训练基线 (c): 1D-CNN on raw waveform (Wen 2018) ...")
    model_c = WaveformCNN1D(in_len=8192).to(DEVICE)
    m_c = _train_and_eval(model_c, "1DCNN_Wen2018_waveform", dl_tr_c, dl_va_c, dl_te_c, lr=5e-4, epochs=30)
